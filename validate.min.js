(function(window,document,undefined){var defaults={messages:{required:'The %{field} field is required.',matches:'The %{field} field does not match the %{param} field.',"default":'The %{field} field is still set to default, please change.',valid_email:'The %{field} field must contain a valid email address.',valid_emails:'The %{field} field must contain all valid email addresses.',min_length:'The %{field} field must be at least %{param} characters in length.',max_length:'The %{field} field must not exceed %{param} characters in length.',exact_length:'The %{field} field must be exactly %{param} characters in length.',greater_than:'The %{field} field must contain a number greater than %{param}.',less_than:'The %{field} field must contain a number less than %{param}.',alpha:'The %{field} field must only contain alphabetical characters.',alpha_numeric:'The %{field} field must only contain alpha-numeric characters.',alpha_dash:'The %{field} field must only contain alpha-numeric characters, underscores, and dashes.',numeric:'The %{field} field must contain only numbers.',integer:'The %{field} field must contain an integer.',decimal:'The %{field} field must contain a decimal number.',is_natural:'The %{field} field must contain only positive numbers.',is_natural_no_zero:'The %{field} field must contain a number greater than zero.',valid_ip:'The %{field} field must contain a valid IP.',valid_base64:'The %{field} field must contain a base64 string.',valid_credit_card:'The %{field} field must contain a valid credit card number.',is_file_type:'The %{field} field must contain only %{param} files.',valid_url:'The %{field} field must contain a valid URL.',greater_than_date:'The %{field} field must contain a more recent date than %{param}.',less_than_date:'The %{field} field must contain an older date than %{param}.',greater_than_or_equal_date:'The %{field} field must contain a date that\'s at least as recent as %{param}.',less_than_or_equal_date:'The %{field} field must contain a date that\'s %{param} or older.'},callback:function(errors){}};var ruleRegex=/^(.+?)\[(.+)\]$/,numericRegex=/^[0-9]+$/,integerRegex=/^\-?[0-9]+$/,decimalRegex=/^\-?[0-9]*\.?[0-9]+$/,emailRegex=/^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$/,alphaRegex=/^[a-z]+$/i,alphaNumericRegex=/^[a-z0-9]+$/i,alphaDashRegex=/^[a-z0-9_\-]+$/i,naturalRegex=/^[0-9]+$/i,naturalNoZeroRegex=/^[1-9][0-9]*$/i,ipRegex=/^((25[0-5]|2[0-4][0-9]|1[0-9]{2}|[0-9]{1,2})\.){3}(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[0-9]{1,2})$/i,base64Regex=/[^a-zA-Z0-9\/\+=]/i,numericDashRegex=/^[\d\-\s]+$/,urlRegex=/^((http|https):\/\/(\w+:{0,1}\w*@)?(\S+)|)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?$/,dateRegex=/\d{4}-\d{1,2}-\d{1,2}/;var FormValidator=function(formNameOrNode,fields,callback){this.callback=callback||defaults.callback;this.errors=[];this.fields={};this.form=this._formByNameOrNode(formNameOrNode)||{};this.messages={};this.handlers={};this.conditionals={};this.i18n=new Polyglot({locale:"en",phrases:defaults.messages});for(var i=0,fieldLength=fields.length;i<fieldLength;i++){var field=fields[i];if((!field.name&&!field.names)||!field.rules){console.warn('validate.js: The following field is being skipped due to a misconfiguration:');console.warn(field);console.warn('Check to ensure you have properly configured a name and rules for this field');continue;}
...
if(failed){var source=this.i18n.phrases[method]||this.messages[field.name+'.'+method]||this.messages[method]||defaults.messages[method],message='An error has occurred with the '+field.display+' field.';if(source){var data={'field':this.i18n.t(field.display)};if(param){data['param']=(this.fields[param])?this.i18n.t(this.fields[param].display):param;}
